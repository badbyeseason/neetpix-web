...
export default function PdfToWordClient() {
  ...
  const convert = useCallback(async () => {
    ...
    setErrorMsg("");
    try {
      const blob = await pdfToWord(file, (currentPage, totalPages) => {
      ...
      a.href = url;
      a.download = "neetpix-converted.docx";
        ...
        >
          ...
          <div className="flex flex-col items-center gap-3">
            <svg className="w-10 h-10 text-text-secondary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 16a4 4 0 01-.88-7.9 5 5 0 019.9-1.49A4.5 4.5 0 0117 16M9 13l3-3 3 3M12 10v9" />
            </svg>
      ...
      {status === "processing" && (
        <div className="flex flex-col items-center justify-center py-20 gap-6">
          ...
          <div className="text-center">
            <p className="text-text-secondary font-medium">{t("converting")}</p>
            {progress && (
              <p className="mt-2 text-sm text-text-secondary">
                {t("pageProgress", { current: progress.current, total: progress.total })}
              </p>
            )}
          </div>
        </div>
      ...
      {status === "done" && (
        <div className="flex flex-col items-center gap-5 py-10">
          ...
          {downloadUrl && (
            <a
              href={downloadUrl}
              download="neetpix-converted.docx"
              className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-teal text-white font-semibold text-sm hover:bg-teal-dark transition-colors"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
