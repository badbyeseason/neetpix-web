...
export default function PdfCropClient() {
  ...
  const crop = useCallback(async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = "neetpix-cropped.pdf";
      ...
      // 根据错误信息映射：边距非法 → errorMargins，其他 → errorParse
      if (err instanceof Error && err.message === "invalid-margins") {
        setErrorMsg(t("errorMargins"));
      } else {
    ...
    <div>
      ...
      {status === "idle" && file && (
        <div className="space-y-6">
          ...
          <div>
                ...
                <div key={key}>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full rounded-xl border border-border bg-bg-warm px-4 py-3 pr-10 text-sm text-text placeholder:text-text-secondary focus:outline-none focus:border-teal focus:ring-1 focus:ring-teal"
                    />
            ...
            <p className="mt-2 text-xs text-text-secondary">{t("marginHint")}</p>
          </div>
      ...
      {status === "processing" && (
        <div className="flex flex-col items-center justify-center py-20 gap-6">
          ...
          <p className="text-text-secondary font-medium">{t("cropping")}</p>
        </div>
      )}
