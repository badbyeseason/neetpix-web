...
export default function PdfEncryptClient() {
  ...
  // 密码校验通过后才可加密
  const canEncrypt =
    password.length >= 4 && password === confirmPassword && !!file;
  ...
  const encrypt = useCallback(async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = "neetpix-encrypted.pdf";
    ...
    <div>
      ...
      {status === "processing" && (
        <div className="flex flex-col items-center justify-center py-20 gap-6">
          ...
          <p className="text-text-secondary font-medium">{t("encrypting")}</p>
        </div>
      )}
      ...
      {status === "done" && (
        <div className="flex flex-col items-center gap-5 py-10">
          ...
          {downloadUrl && (
            <a
              href={downloadUrl}
              download="neetpix-encrypted.pdf"
